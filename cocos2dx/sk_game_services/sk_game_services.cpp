...
void ios_show_banner_ad( bool isVisible );
...
void ios_tapjoy_open_offer_wall();
std::string ios_get_gc_player_id();
bool ios_is_gc_authenticated();
...
bool is_unique_userid_authenticated()
{
    #if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
        return ios_is_gc_authenticated();
    #endif
    
    #if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
        CCLOG("DNA before android is gs authenticated");
        return android_is_gs_authenticated();
    #endif
    
    return false;
}

std::string get_unique_userid()
{
    #if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
        return ios_get_gc_player_id();
    #endif
    
    #if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
        CCLOG("DNA before android get gs player id ");
        return android_get_gs_player_id();
    #endif
...
void on_level_ended()
{
	...
	if (num_level_ended_per_ad() < 1)
	{
		CCMessageBox("Error", "num_level_ended_per_ad must be >= 1");
	}
	if (num_played == num_level_ended_per_ad() - 1)
	{
		num_played = 0;
	}
	else
...
//
	...
	int num_level_ended_per_ad()
	{
		return GameServicesBase::GetGameServices()->num_level_ended_per_ad();
	}

	int num_video_ad_per_interstitial()
	{
		return GameServicesBase::GetGameServices()->num_video_ad_per_interstitial();
	}
