...
bool GAFAsset::initWithImageData(const std::string& jsonPath)
{
	...
	{
		CCLOGERROR("CAN NOT create GAFAsset : %s does not exists", jsonPath.c_str());
		return false;
	}
	GAFData aConfigData;
	...
	if (!file)
	{
		CCLOGERROR("CAN NOT create GAFAsset : can not open %s.", fp.c_str());
		return false;
	}
	
	aConfigData.size = s3eFileGetSize(file);
	aConfigData.ptr = new unsigned char[aConfigData.size];
	s3eFileRead(aConfigData.ptr, aConfigData.size, 1, file);	
	s3eFileClose(file);
...
{
	IwAssert(_objects, "objects != NULL");
	...
	for (unsigned int index = 0; index < configFrameCount; ++index)
	{
		if (configFrameIndex < animationConfigFrames->count())
		{
			...
			unsigned int configFrameNo = configFrameNoInt ? configFrameNoInt->getValue() : -1;
